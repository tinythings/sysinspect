...
impl SysInspectUX {
    ...
    pub fn dialog_master_logs(&self, parent: Rect, buf: &mut Buffer) {
        ...
        Self::_render_logs_filter(filter_area, buf, self.master_logs_filter_focus, &self.master_logs_filter);

        let path_line = Line::styled(format!(" {} ", section.path), Style::default().fg(palette::MUTED).bg(bg).add_modifier(Modifier::DIM));
        Widget::render(Paragraph::new(path_line), path_area, buf);

        let filter_val = self.master_logs_filter.value().to_lowercase();
        let filtered_lines: Vec<&String> = if filter_val.is_empty() {
            section.lines.iter().collect()
        } else {
            section.lines.iter().filter(|l| l.to_lowercase().contains(&filter_val)).collect()
        };

        let text_area = Rect { x: content_area.x, y: content_area.y, width: content_area.width.saturating_sub(1), height: content_area.height };
        self.master_logs_viewport_rows.set(text_area.height as usize);

        let rendered_lines = Self::filtered_master_rendered_lines(&section.lines, &self.master_logs_filter);
        let text_h = text_area.height as usize;
        let max_top = rendered_lines.len().saturating_sub(text_h);
        let scroll = section.scroll.get();
        let start = if scroll == usize::MAX { max_top } else { scroll.min(max_top) };
        let end = (start + text_h).min(rendered_lines.len());
        if filtered_lines.is_empty() {
            let msg = if section.lines.is_empty() { "(no log lines)" } else { "(no matches)" };
            Widget::render(Paragraph::new(msg).style(Style::default().fg(palette::FG).bg(bg)), text_area, buf);
        } else {
            let visible: Vec<Line> = rendered_lines[start..end].to_vec();
            Widget::render(Paragraph::new(visible).style(Style::default().bg(bg)), text_area, buf);
        ...
        let scroll_area = Rect { x: content_area.right().saturating_sub(1), y: content_area.y, width: 1, height: content_area.height };
        let mut scrollbar = ScrollbarState::default().content_length(rendered_lines.len().max(1)).position(start);
        Scrollbar::default()
            .begin_symbol(None)
            .end_symbol(None)
            .track_symbol(Some("\u{28FF}"))
            ...
            .track_style(Style::default().bg(bg))
            .thumb_style(Style::default().fg(palette::GRAY_1))
            .render(scroll_area, buf, &mut scrollbar);
    ...
    pub(crate) fn filtered_master_rendered_lines(lines: &[String], filter: &InputState) -> Vec<Line<'static>> {
        let fv = filter.value().to_lowercase();
        let filtered: Vec<&String> =
            if fv.is_empty() { lines.iter().collect() } else { lines.iter().filter(|l| l.to_lowercase().contains(&fv)).collect() };
        filtered.iter().flat_map(|s| render_log_line(s)).collect()
    }
