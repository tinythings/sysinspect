...
pub fn cli(version: &'static str) -> Command {
    ...
    Command::new(APPNAME)
        ...
        .subcommand(Command::new("network").about("Manage cluster transport state and rotation").styles(styles.clone()).disable_help_flag(true)
            .arg(Arg::new("add").short('A').long("add").action(ArgAction::SetTrue).help("Plan onboarding for one or more hosts").conflicts_with_all(["rotate", "status", "online", "info"]))
            ...
            .arg(Arg::new("names").long("names").help("Comma-separated host onboarding entries").conflicts_with("list"))
            .arg(Arg::new("list").long("list").help("Path to a file containing one onboarding entry per line").conflicts_with("names"))
            .arg(Arg::new("user").long("user").help("Default SSH login user for host onboarding"))
            ...
            .arg(Arg::new("id").long("id").help("Target a specific minion by its system id").conflicts_with_all(["query", "query-pos", "add", "names", "list", "user"]))
            .arg(Arg::new("query").long("query").help("Target minions by hostname glob or query").conflicts_with_all(["query-pos", "add", "names", "list", "user"]))
            .arg(Arg::new("select-traits").long("traits").help("Target minions by traits query").conflicts_with_all(["add", "names", "list", "user"]))
