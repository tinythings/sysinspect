...
pub fn cli(version: &'static str) -> Command {
    ...
    Command::new(APPNAME)
        ...
        .subcommand(Command::new("network").about("Manage cluster transport state and rotation").styles(styles.clone()).disable_help_flag(true)
            ...
            .arg(Arg::new("online").short('o').long("online").action(ArgAction::SetTrue).help("Show online minions for the current selection").conflicts_with_all(["rotate", "status", "info"]))
            .arg(Arg::new("info").long("info").action(ArgAction::SetTrue).help("Show detailed minion registry information for exactly one minion selected by name or --id").conflicts_with_all(["rotate", "status", "online"]))
