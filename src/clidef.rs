...
pub fn cli(version: &'static str) -> Command {
    ...
    Command::new(APPNAME)
        ...
        .subcommand(Command::new("network").about("Manage cluster transport state and rotation").styles(styles.clone()).disable_help_flag(true)
            .arg(Arg::new("add").short('A').long("add").action(ArgAction::SetTrue).help("Plan onboarding for one or more hosts").conflicts_with_all(["remove", "upgrade", "rotate", "status", "info"]))
            .arg(Arg::new("remove").short('R').long("remove").action(ArgAction::SetTrue).help("Remove one or more managed hosts").conflicts_with_all(["add", "upgrade", "rotate", "status", "info"]))
            .arg(Arg::new("upgrade").long("upgrade").action(ArgAction::SetTrue).help("Upgrade one or more locally managed hosts").conflicts_with_all(["add", "remove", "rotate", "status", "info"]))
            .arg(Arg::new("force").short('f').long("force").action(ArgAction::SetTrue).help("For --remove, forget the minion on the master even when the host cannot be reached. For --add, remove any existing managed install first, then add again.").conflicts_with_all(["upgrade", "rotate", "status", "info"]))
            ...
            .arg(Arg::new("info").long("info").action(ArgAction::SetTrue).help("Show detailed minion registry information for exactly one minion selected by name or --id").conflicts_with_all(["add", "rotate", "status"]))
