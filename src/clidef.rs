...
pub fn cli(version: &'static str) -> Command {
    ...
    Command::new(APPNAME)
        ...
        .subcommand(Command::new("network").about("Manage cluster transport state and rotation").styles(styles.clone()).disable_help_flag(true)
            ...
            .arg(Arg::new("force").short('f').long("force").action(ArgAction::SetTrue).help("Force host removal to forget the minion on the master even when the host cannot be reached").conflicts_with_all(["add", "upgrade", "rotate", "status", "online", "info"]))
