...
pub(crate) fn detect_backend() -> Option<&'static str> {
    ...
    candidates.iter().find_map(|(name, args)| {
        Command::new(args[0]).args(&args[1..]).output().ok().filter(|o| o.status.success()).map(|_| *name)
    })
}
...
fn translate_rule(backend: &str, args: &serde_json::Value) -> Result<Vec<String>, String> {
    ...
    match backend {
        "pf" => {
            ...
            if !port.is_empty() { rule.push_str(&format!(" port {}", port)); }
        ...
        "ipfw" => {
            ...
            if src != "any" { rule.push_str(&format!(" from {}", src)); } else { rule.push_str(" from any"); }
            if dst != "any" { rule.push_str(&format!(" to {}", dst)); } else { rule.push_str(" to any"); }
            if !port.is_empty() { rule.push_str(&format!(" dst-port {}", port)); }
        ...
        "nftables" => {
            ...
            if proto != "any" { rule.push_str(&format!(" {} dport {}", proto, port)); }
