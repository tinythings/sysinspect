...
impl MeNotifyModuleRef {
    ...
    pub fn new(listener: &str) -> Result<Self, MeNotifyError> {
        listener.split_once('.').ok_or_else(|| MeNotifyError::MissingModule(listener.to_string())).and_then(|(root, module)| {
            if root != "menotify" {
            ...
            Ok(Self { listener: listener.to_string(), module: module.trim().to_string() })
        })
    }
