...
.PHONY: setup help release build dev all all-dev modules modules-dev modules-dist-dev modules-refresh-dev \
...
help release build dev all all-dev modules modules-dev modules-dist-dev modules-refresh-dev modules-refresh clean check fix stats man test test-core test-modules test-sensors test-integration tar dev-tls musl-aarch64-dev musl-aarch64 musl-x86_64-dev musl-x86_64:
	@command -v ${GNU_MAKE} >/dev/null 2>&1 || { \
